...
import {
  NiivueEvent,
  type NiivueEventListener,
  type NiivueEventListenerOptions,
  type NiivueEventMap,
} from "@/events"
import defaultFontMetrics from "@/fonts/Roboto-Regular.json" with {
  type: "json",
}
...
const { LEFT_MOUSE_BUTTON, CENTER_MOUSE_BUTTON, RIGHT_MOUSE_BUTTON } =
  MouseController
...
export class Niivue extends EventTarget {
  ...
  meshShaders: Array<{ Name: string; Frag: string; shader?: Shader }> =
    ShaderManager.createDefaultMeshShaders()
   ...
   * @private
   */
  private _emitEvent<K extends keyof NiivueEventMap>(
    eventName: K,
  ...
  cleanup(): void {
    ...
    EventController.cleanupResizeObservers(
      this.resizeObserver,
      this.canvasObserver,
  ...
  async attachToCanvas(
    ...
    log.debug(
      `Max texture size 2D: ${this.uiData.max2D} 3D: ${this.uiData.max3D}`,
    )
    ...
    if (this.opts.isResizeCanvas) {
      ...
      this.resizeEventListener = EventController.createResizeHandler(() =>
        this.resizeListener(),
      )
      window.addEventListener("resize", this.resizeEventListener)
      this.resizeObserver = EventController.createResizeObserver(() =>
        this.resizeListener(),
      )
      this.resizeObserver.observe(this.canvas.parentElement!)

      // Setup a MutationObserver to detect when canvas is removed from DOM
      this.canvasObserver = EventController.createCanvasObserver(
        (mutations) => {
  ...
  getNoPaddingNoBorderCanvasRelativeMousePosition(
    ...
    return EventController.getNoPaddingNoBorderCanvasRelativeMousePosition(
      event,
      target,
    )
  }
    ...
    if (dragMode === DRAG_MODE.crosshair) {
      ...
      if (dragMode === DRAG_MODE.angle) {
        if (this.uiData.angleState === "none") {
          ...
          const finalClickPos = [
            pos.x * this.uiData.dpr!,
            pos.y * this.uiData.dpr!,
          ]
          ...
          const firstLineStartFrac = this.canvasPos2frac([
            this.uiData.angleFirstLine[0],
            this.uiData.angleFirstLine[1],
          ])
          const firstLineEndFrac = this.canvasPos2frac([
            this.uiData.angleFirstLine[2],
            this.uiData.angleFirstLine[3],
          ])
          const secondLineStartFrac = this.canvasPos2frac([
            secondLine[0],
            secondLine[1],
          ])
          const secondLineEndFrac = this.canvasPos2frac([
            secondLine[2],
            secondLine[3],
          ])
          ...
          if (
            ...
            const angleToSave = {
              ...
              angle: this.calculateAngleBetweenLines(
      ...
      this.uiData.dragClipPlaneStartDepthAziElev =
        this.scene.clipPlaneDepthAziElevs[this.uiData.activeClipPlaneIndex]
    }
  }
  ...
  calculateNewRange({ volIdx = 0 } = {}): void {
    ...
    if (
      !MouseController.hasDragMoved(this.uiData.dragStart, this.uiData.dragEnd)
    ) {
      return
    }
    // calculate our box
    ...
    const adjustedRanges = DragModeManager.adjustRangesForConstantDimension(
      startVox,
      endVox,
      xrange,
      yrange,
  ...
  generateMouseUpCallback(fracStart: vec3, fracEnd: vec3): void {
    ...
    const tileStart = this.tileIndex(
      this.uiData.dragStart[0],
      this.uiData.dragStart[1],
    )
    const tileEnd = this.tileIndex(
    ...
    vec3.sub(
      v,
      vec3.fromValues(mmStart[0], mmStart[1], mmStart[2]),
      vec3.fromValues(mmEnd[0], mmEnd[1], mmEnd[2]),
    )
  ...
  mouseUpListener(): void {
    ...
    if (this.drawPenFillPts.length > 0) {
      ...
      (this.opts.penType === PEN_TYPE.RECTANGLE ||
        this.opts.penType === PEN_TYPE.ELLIPSE)
    ) {
    ...
    if (this.uiData.isDragging) {
      ...
      const fracStart = this.canvasPos2frac([
        this.uiData.dragStart[0],
        this.uiData.dragStart[1],
      ])
      const fracEnd = this.canvasPos2frac([
        this.uiData.dragEnd[0],
        this.uiData.dragEnd[1],
      ])
      ...
      if (currentDragMode === DRAG_MODE.contrast) {
        ...
        if (
          !MouseController.hasDragMoved(
            this.uiData.dragStart,
            this.uiData.dragEnd,
          )
        ) {
          this.clearActiveDragMode()
      ...
      if (currentDragMode === DRAG_MODE.measurement) {
        ...
        // Convert canvas coordinates to world coordinates
        const startFrac = this.canvasPos2frac([
          this.uiData.dragStart[0],
          this.uiData.dragStart[1],
        ])
        const endFrac = this.canvasPos2frac([
          this.uiData.dragEnd[0],
          this.uiData.dragEnd[1],
        ])
  ...
  touchStartListener(e: TouchEvent): void {
    ...
    if (doubleTapResult.isDoubleTap) {
      ...
      const newSequenceState =
        TouchController.createNewTouchSequenceState(currentTime)
    ...
    if (
      TouchController.isSingleFingerTouch(
        this.uiData.touchdown,
        e.touches.length,
      )
    ) {
  ...
  mouseLeaveListener(): void {
    ...
    if (this.opts.drawingEnabled && !isNaN(this.drawShapeStartLocation[0])) {
      log.debug(
        "Mouse left canvas during shape drawing, resetting shape state.",
      )
      this.drawShapeStartLocation = [NaN, NaN, NaN]
  ...
  mouseMoveListener(e: MouseEvent): void {
    ...
    if (this.uiData.mousedown) {
      ...
      this.getCurrentDragMode() === DRAG_MODE.angle &&
      this.uiData.angleState === "drawing_second_line"
    ) {
  ...
  resetBriCon(msg: TouchEvent | MouseEvent | null = null): void {
    ...
    if (msg !== null) {
      ...
      if ("targetTouches" in msg) {
        x =
          msg.targetTouches[0].clientX -
          (msg.target as HTMLElement).getBoundingClientRect().left
        y =
          msg.targetTouches[0].clientY -
          (msg.target as HTMLElement).getBoundingClientRect().top
      } else {
        // if a mouse event
        x = msg.offsetX
        y = msg.offsetY
  ...
  setDragStart(x: number, y: number): void {
    const [scaledX, scaledY] = DragModeManager.calculateDragPosition(
      x,
      y,
      this.uiData.dpr!,
    )
  ...
  handlePinchZoom(e: TouchEvent): void {
    if (
      TouchController.shouldProcessPinchZoom(
  ...
  keyUpListener(e: KeyboardEvent): void {
    ...
    if (
      KeyboardController.isHotkeyMatch(e.code, this.opts.cycleClipPlaneHotKey)
    ) {
      if (shouldProcess) {
    ...
    if (KeyboardController.isHotkeyMatch(e.code, this.opts.clipPlaneHotKey)) {
      ...
      KeyboardController.isHotkeyMatch(e.code, this.opts.viewModeHotKey)
    ) {
      if (shouldProcess) {
  ...
  keyDownListener(e: KeyboardEvent): void {
    ...
    switch (action) {
      ...
      case "cycle_drag_mode":
        this.opts.dragMode = KeyboardController.getNextDragMode(
          this.opts.dragMode,
        )
  ...
  wheelListener(e: WheelEvent): void {
    ...
    if (
      ...
      const tileIdx = this.tileIndex(
        this.uiData.dragStart[0],
        this.uiData.dragStart[1],
      )
      if (tileIdx >= 0) {
        this.generateMouseUpCallback(
          this.screenXY2TextureFrac(
            this.uiData.dragStart[0],
            this.uiData.dragStart[1],
            tileIdx,
          ),
        ...
        log.warn(
          "Could not generate drag release callback for ROI selection: Invalid tile index.",
        )
      }
      return
    }
    ...
    if (this.opts.clickToSegment) {
      ...
      if (
        tileIdx >= 0 &&
        this.screenSlices[tileIdx].axCorSag <= SLICE_TYPE.SAGITTAL
      ) {
        log.debug(
    ...
    const rect = this.canvas!.getBoundingClientRect()
    const [x, y] = WheelController.getWheelEventPosition(
      e.clientX,
      e.clientY,
      rect,
    )

    const isInRenderTile =
    ...
    if (
      ...
      const panOffset = WheelController.calculatePanOffsetAfterZoom({
        currentPan: [
          this.scene.pan2Dxyzmm[0],
          this.scene.pan2Dxyzmm[1],
          this.scene.pan2Dxyzmm[2],
        ],
  ...
  async loadFromFile(file: File): Promise<void> {
    ...
    if (MESH_EXTENSIONS.includes(ext)) {
      await NVMesh.loadFromFile({ file, gl: this.gl, name: file.name }).then(
        (mesh) => {
          this.addMesh(mesh)
        },
      )
   ...
   * @internal
   */
  private async handleVolumeFileDrop(
    file: File,
    pairedEntry: FileSystemEntry | null,
    altKey: boolean,
  ): Promise<void> {
    try {
      ...
      if (pairedEntry) {
        pairedFile = await DropHandler.getFileFromEntry(
          pairedEntry as FileSystemFileEntry,
        )
      }
   ...
   * @internal
   */
  private async handleDirectoryDrop(
  ...
  drawClearAllUndoBitmaps(): void {
    const result = DrawingManager.clearAllUndoBitmaps(
      this.drawUndoBitmaps,
      this.opts.maxDrawUndoBitmaps,
    )
    this.drawUndoBitmaps = result.drawUndoBitmaps
    this.currentDrawUndoBitmap = result.currentDrawUndoBitmap
  }
  ...
  loadDrawing(drawingBitmap: NVImage): boolean {
    ...
    if (this.opts.is2DSliceShader) {
      this.drawTexture = this.r8Tex2D(
        this.drawTexture,
        TEXTURE_CONSTANTS.TEXTURE7_DRAW,
        this.back.dims,
        true,
      )
    } else {
...
if (perm[0] === 1 && perm[1] === 2 && perm[2] === 3) {
  ...
  sliceScroll2D(posChange: number, x: number, y: number, isDelta = true): void {
    ...
    if (
      ...
      log.warn(
        "Canvas element does not have focus. Scroll events will not be processed.",
      )
      return
    }
  ...
  setClipPlaneThick(_thick: number): void {
    log.warn(
      "setClipPlaneThick() has been removed. use setClipPlanes() instead.",
    )
  }
  ...
  async loadDocument(document: NVDocument): Promise<this> {
    ...
    try {
      const images: NVImage[] = await NVSerializer.rehydrateImages(
        document.data,
      )
    ...
    try {
      const meshes: NVMesh[] = await NVSerializer.rehydrateMeshes(
        document.data,
        this.gl,
        /* callUpdateMesh= */ true,
      )
    ...
    if (drawingBase64) {
      try {
        ...
        if (drawingBitmap) {
          ...
          if (!dims) {
            console.warn(
              "No back image available; cannot validate drawBitmap size.",
            )
...
*/
  async generateLoadDocumentJavaScript(
    canvasId: string,
    esm: string,
  ): Promise<string> {
    const json = this.json()

    const base64 = await NVUtilities.compressToBase64String(
      JSON.stringify(json),
    )
    const javascript = `
        ${esm}
   ...
   */
  async loadImages(
    images: Array<ImageFromUrlOptions | LoadFromUrlParams>,
  ): Promise<this> {
    const volumes = []
    const meshes = []
  ...
  async loadDicoms(dicomList: ImageFromUrlOptions[]): Promise<this> {
    ...
    if (!this.getDicomLoader()) {
      throw new Error(
        "No dicom loader set. Please set a dicom loader before loading dicoms",
      )
    }
    this.drawScene()
    this.volumes = []
  ...
  async addMeshesFromUrl(meshOptions: LoadFromUrlParams[]): Promise<NVMesh[]> {
    const promises = meshOptions.map(async (meshItem) => {
      ...
      if (this.loaders[ext]) {
        ...
        const mz3 = await NVMeshUtilities.createMZ3Async(
          positions,
          indices,
          false,
          colors,
   ...
   * @see {@link https://niivue.com/demos/features/connectome.html | live demo usage}
   */
  async loadFreeSurferConnectomeFromUrl(
    url: string,
    headers = {},
  ): Promise<this> {
    const response = await fetch(url, { headers })
  ...
  handleNodeAdded(event: { detail: { node: NVConnectomeNode } }): void {
    const labelData = ConnectomeManager.createNodeAddedLabelData(
      event.detail.node,
    )
  ...
  drawGrowCut(): void {
    ...
    const background = this.r16Tex(
      null,
      TEXTURE_CONSTANTS.TEXTURE11_GC_BACK,
      this.back.dims,
      img16,
    )
    for (let i = 1; i < nv; i++) {
    ...
    const label0 = this.r16Tex(
      null,
      TEXTURE_CONSTANTS.TEXTURE14_GC_LABEL0,
      this.back.dims,
      img16,
    )
    const label1 = this.r16Tex(
      null,
      TEXTURE_CONSTANTS.TEXTURE15_GC_LABEL1,
      this.back.dims,
      img16,
    )
    ...
    const strength0 = this.r16Tex(
      null,
      TEXTURE_CONSTANTS.TEXTURE12_GC_STRENGTH0,
      this.back.dims,
      img16,
    )
    const strength1 = this.r16Tex(
      null,
      TEXTURE_CONSTANTS.TEXTURE13_GC_STRENGTH1,
      this.back.dims,
      img16,
    )
  ...
  drawFloodFill(
    ...
    if (this.clickToSegmentIsGrowing && !this.clickToSegmentGrowingBitmap) {
      log.warn(
        "drawFloodFill called in preview mode without initialized clickToSegmentGrowingBitmap.",
      )
      if (this.drawBitmap) {
    ...
    const originalBitmap = this.clickToSegmentIsGrowing
      ? this.drawBitmap
      : targetBitmap
    if (!originalBitmap) {
    ...
    const constrainXYZ = FloodFillTool.getConstrainedAxisIndex(
      is2D,
      this.drawPenAxCorSag,
    )
    ...
    if (
      isGrowClusterTool &&
      (growSelectedCluster === Number.POSITIVE_INFINITY ||
        growSelectedCluster === Number.NEGATIVE_INFINITY)
    ) {
      const tempImgForIdentification = originalBitmap.slice()
      for (let i = 0; i < nxyz; i++) {
        tempImgForIdentification[i] =
          tempImgForIdentification[i] === originalSeedColor &&
          isWithinDistance(i)
            ? 1
            : 0
      }
      ...
      log.debug(
        `Grow Cluster using mean intensity: ${baseIntensity.toFixed(2)} voxels.`,
      )
      ...
      if (growSelectedCluster === 0) {
        if (isGrowClusterTool && newColor === 0) {
          log.debug(
            `Erase Cluster: Identifying cluster with color ${originalSeedColor}`,
          )
          for (let i = 0; i < nxyz; i++) {
            img[i] =
              originalBitmap[i] === originalSeedColor && isWithinDistance(i)
                ? 1
                : 0
          }
        } else {
          for (let i = 0; i < nxyz; i++) {
            if (
              originalBitmap[i] === originalSeedColor &&
              isWithinDistance(i)
            ) {
    ...
    if (img[seedVx] !== 1) {
      ...
      if (isSeedValidOriginal && isWithinDistance(seedVx)) {
        ...
        log.debug(
          "Seed voxel not marked as candidate '1' and not valid originally.",
        )
        if (
          this.clickToSegmentIsGrowing &&
          this.clickToSegmentGrowingBitmap &&
    ...
    FloodFillTool.applyFillResult({
      ...
      isPreviewMode:
        this.clickToSegmentIsGrowing &&
        targetBitmap === this.clickToSegmentGrowingBitmap,
    })
  ...
  refreshDrawing(isForceRedraw = true, useClickToSegmentBitmap = false): void {
    ...
    if (!this.back?.dims) {
      ...
      log.warn(
        "refreshDrawing: back.dims undefined, cannot refresh drawing texture yet.",
      )
      return
    }
    ...
    const dims = DrawingManager.adjustDimensionsForSpecialCase(
      bitmapDataSource.length,
      this.back.dims.slice(),
    )
    if (!DrawingManager.validateBitmapLength(bitmapDataSource.length, vx)) {
      log.warn(
        `Drawing bitmap length (${bitmapDataSource.length}) must match the background image (${vx})`,
      )
    }
    ...
    if (this.opts.is2DSliceShader) {
      ...
      log.debug(
        `refresh huge 2D drawing x×y×z ${dims[1]}×${dims[2]}×${dims[3]} slice ${zOffset}`,
      )
  ...
  closePAQD(): void {
    ...
    this.paqdTexture = this.rgbaTex(
      this.paqdTexture,
      TEXTURE_CONSTANTS.TEXTURE8_PAQD,
      [2, 2, 2, 2],
      true,
    )
  }
  ...
  meshShaderNameToNumber(meshShaderName = "Phong"): number | undefined {
    return ShaderManager.meshShaderNameToNumber(
      meshShaderName,
      this.meshShaders,
    )
  }
  ...
  setMeshShader(
    id: number | string,
    meshShaderNameOrNumber: number | string = 2,
  ): void {
  ...
  async setCustomMeshShaderFromUrl(url: string, name = ""): Promise<number> {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch shader from ${url}: ${response.status} ${response.statusText}`,
        )
      }
      ...
      throw new Error(
        `setCustomMeshShaderFromUrl(${url}) failed: ${(err as Error).message}`,
      )
    }
  }
  ...
  async init(): Promise<this> {
    ...
    this.volumeTexture = this.rgbaTex(
      this.volumeTexture,
      TEXTURE_CONSTANTS.TEXTURE0_BACK_VOL,
      [2, 2, 2, 2],
      true,
    )
    this.overlayTexture = this.rgbaTex(
      this.overlayTexture,
      TEXTURE_CONSTANTS.TEXTURE2_OVERLAY_VOL,
      [2, 2, 2, 2],
      true,
    )
    this.drawTexture = this.r8Tex(
    ...
    const orientCubeShaderResult = ShaderManager.initOrientCubeShader(
      gl,
      orientCube,
      this.unusedVAO,
    )
    ...
    this.renderGradientValuesShader =
      volumeRenderShaders.renderGradientValuesShader
   ...
   * @param skipGradientGL - when true, skip the Sobel-gradient computation
   *   inside {@link refreshLayers}. The gradient texture is needed only for
   *   volume-render illumination ({@link setVolumeRenderIllumination}) and
   *   gradient-opacity effects ({@link setGradientOpacity}). Pass `true`
   *   during progressive/streaming volume loads to avoid running
   *   2×(depth-1) GPU draw calls (one per slice for blur + one for Sobel)
   *   synchronously, which can trigger the browser's GPU watchdog on large
   *   volumes. The gradient will be recomputed automatically the next time
   *   a gradient-dependent method is called.
   * @example
   * niivue = new Niivue()
   * niivue.updateGLVolume()
   * @see {@link https://niivue.com/demos/features/colormaps.html | live demo usage}
   */
  updateGLVolume(skipGradientGL = false): void {
    ...
    for (let i = 0; i < numLayers; i++) {
      ...
      this.refreshLayers(this.volumes[i], visibleLayers, skipGradientGL)
      visibleLayers++
    }
    this.furthestVertexFromOrigin = 0.0
    if (numLayers > 0) {
      this.furthestVertexFromOrigin =
        this.volumeObject3D?.furthestVertexFromOrigin ?? 0
  ...
  getDescriptives(options: {
    ...
    const maskImages = masks
      .filter((m) => {
        ...
        if (!valid) {
          log.debug(
            "Mask resolution does not match image. Skipping masking layer " + m,
          )
        }
        return valid
      })
    ...
    return {
      ...
      volumeML:
        stats.nvox * hdr.pixDims[1] * hdr.pixDims[2] * hdr.pixDims[3] * 0.001,
   ...
   * @param skipGradientGL - when true, skip the Sobel-gradient computation
   *   for layer 0. See {@link updateGLVolume} for details.
   * @internal
   */
  refreshLayers(
    overlayItem: NVImage,
    layer: number,
    skipGradientGL = false,
  ): void {
    if (this.volumes.length < 1) {
      return
    } // e.g. only meshes
    ...
    if (layer === 0) {
      ...
      if (isAboveMax3D) {
        ...
        const scale =
          (255 * hdr.scl_slope) / (overlayItem.cal_max - overlayItem.cal_min)
        const intercept =
          (255 * (hdr.scl_inter - overlayItem.cal_min)) /
          (overlayItem.cal_max - overlayItem.cal_min)
    ...
    if ("alphaThreshold" in overlayItem) {
      ...
      if (overlayItem.alphaThreshold === true) {
        overlayItem.colormapType =
          COLORMAP_TYPE.ZERO_TO_MAX_TRANSLUCENT_BELOW_MIN
      }
      if (overlayItem.alphaThreshold === false) {
        overlayItem.colormapType =
          COLORMAP_TYPE.ZERO_TO_MAX_TRANSPARENT_BELOW_MIN
      }
  ...
  async createConnectedLabelImage(
    ...
    const [mx, clusterImg] = this.bwlabel(
      img,
      dim!,
      conn,
      binarize,
  ...
  async createNiftiArray(
    ...
    return await NVImage.createNiftiArray(
      dims,
      pixDims,
      affine,
      datatypeCode,
  ...
  async conform(
    ...
    const inNvox =
      volume.hdr!.dims![1] * volume.hdr!.dims![2] * volume.hdr!.dims![3]
    ...
    if (rawFloat32) {
      ...
      const outImg32 = ImageProcessing.scalecropFloat32(
        outImg,
        0,
        1,
        srcMin,
      ...
      bytes = await this.createNiftiArray(
        ...
        Array.from(outAffine),
        NiiDataType.DT_FLOAT32,
        new Uint8Array(outImg32.buffer),
      )
    } else {
      // Compute intensity scale factors
      const f_low = isRobustMinMax ? NaN : 0
      ...
      const [srcMin, scale] = ImageProcessing.getScale(scaleParams)
      const outImg8 = ImageProcessing.scalecropUint8(
        outImg,
        0,
        255,
        srcMin,
  ...
  setRenderDrawAmbientOcclusion(ao: number): void {
    ...
    this.renderShader.use(this.gl)
    this.gl.uniform1fv(this.renderShader.uniforms.renderDrawAmbientOcclusion, [
  ...
  refreshColormaps(): this | undefined {
    ...
    if (nVol > 0) {
      ...
      for (let i = 0; i < nVol; i++) {
        ...
        const isColorbarFromZero =
          volume.colormapType !== COLORMAP_TYPE.MIN_TO_MAX
    ...
    if (nmesh > 0) {
      ...
      for (let i = 0; i < nmesh; i++) {
        ...
        if (
          "edgeColormap" in mesh &&
          "edges" in mesh &&
          mesh.edges !== undefined
        ) {
        ...
        for (let j = 0; j < nlayers; j++) {
          ...
          const isColorbarFromZero =
            layer.colormapType !== COLORMAP_TYPE.MIN_TO_MAX
    ...
    for (let i = 0; i < nMaps; i++) {
      addColormap(
        Array.from(
          this.colormap(
            this.colormapLists[i].name,
            this.colormapLists[i].invert,
          ),
        ),
      )
    }
  ...
  sliceScale(forceVox = false): SliceScale {
    ...
    return LayoutManager.calculateSliceScale({
      forceVox,
      getScreenFieldOfViewMM: (axCorSag: SLICE_TYPE) =>
        this.screenFieldOfViewMM(axCorSag),
      getScreenFieldOfViewVox: (axCorSag: SLICE_TYPE) =>
        this.screenFieldOfViewVox(axCorSag),
      backDims: this.back.dims,
    })
  }
  ...
  sliceScroll3D(posChange = 0): void {
    ...
    if (
      result.action === "clipPlane" &&
      result.newClipPlaneDepth !== undefined
    ) {
      const depthAziElev =
        this.scene.clipPlaneDepthAziElevs[
    ...
    if (
      result.action === "zoom" &&
      result.newVolScaleMultiplier !== undefined
    ) {
  ...
  inGraphTile(x: number, y: number): boolean {
    return SliceNavigation.isInGraphTile({
      ...
      graph: this.graph,
      volumesLength: this.volumes.length,
      firstVolumeFrameCount:
        this.volumes.length > 0 ? (this.volumes[0].nFrame4D ?? 0) : 0,
    })
  }
  ...
  doClickToSegment(options: { x: number; y: number; tileIndex: number }): void {
    ...
    const brightOrDark = FloodFillTool.getGrowDirection(
      intensityResult.isBright,
    )

    this.drawPenAxCorSag = axCorSag

    // Determine which bitmap to pass to drawFloodFill
    const targetBitmap = this.clickToSegmentIsGrowing
      ? this.clickToSegmentGrowingBitmap
      : this.drawBitmap
    ...
    if (!this.clickToSegmentIsGrowing) {
      ...
      if (this.drawBitmap) {
        ...
        log.error(
          "Cannot refresh drawing after click-to-segment apply, drawBitmap is null.",
        )
      }
  ...
  mouseClick(x: number, y: number, posChange = 0, isDelta = true): boolean {
    ...
    for (let i = 0; i < this.screenSlices.length; i++) {
      ...
      // lag
      const nextPos = this.opts.isForceMouseClickToVoxelCenters
        ? this.vox2frac(this.frac2vox(texFrac))
        : texFrac
      ...
      if (this.opts.drawingEnabled) {
        const pt = this.frac2vox(this.scene.crosshairPos) as [
          number,
          number,
          number,
        ]
              ...
              if (this.drawShapePreviewBitmap && this.drawBitmap) {
                ...
                if (this.opts.penType === PEN_TYPE.RECTANGLE) {
                  this.drawRectangleMask(
                    this.drawShapeStartLocation,
                    pt,
                    this.opts.penValue,
                  )
                } else if (this.opts.penType === PEN_TYPE.ELLIPSE) {
                  this.drawEllipseMask(
                    this.drawShapeStartLocation,
                    pt,
                    this.opts.penValue,
                  )
  ...
  drawRuler(): void {
    ...
    const outlineColor = UIElementRenderer.getRulerOutlineColor(
  ...
  drawRuler10cm(
    ...
    const ticks = UIElementRenderer.calculateRulerTicks(
      startXYendXY,
  ...
  drawAngleTextForAngle(angle: {
    ...
    const angle_degrees = this.calculateAngleBetweenLines(
      angle.firstLine,
      angle.secondLine,
    )
  ...
  drawRect(leftTopWidthHeight: number[], lineColor = [1, 0, 0, -1]): void {
    ...
    if (!this.opts.selectionBoxIsOutline) {
      ...
      this.gl.uniform1f(
        this.rectOutlineShader.uniforms.thickness,
        this.opts.selectionBoxLineThickness,
      )
  ...
  private drawBoundsBox(
    ...
    gl.uniform2fv(this.rectOutlineShader.uniforms.canvasWidthHeight, [
      gl.canvas.width,
      gl.canvas.height,
    ])
  ...
  effectiveCanvasHeight(): number {
    return LayoutManager.calculateEffectiveCanvasHeight({
      ...
      bounds: this.opts.bounds as
        | LayoutManager.NormalizedBounds
        | null
        | undefined,
  ...
  getBulletMarginWidth(): number {
    ...
    return LayoutManager.calculateBulletMarginWidth({
      labels,
      fontPx: this.fontPx,
      textHeight: (fontSize: number, text: string) =>
        this.textHeight(fontSize, text),
    })
  }
  ...
  getLegendPanelWidth(): number {
    ...
    return LayoutManager.calculateLegendPanelWidth({
      ...
      fontPx: this.fontPx,
      canvasWidth: this.gl.canvas.width,
      textWidth: (fontSize: number, text: string) =>
        this.textWidth(fontSize, text),
  ...
  drawText(
    ...
    let screenPxRange =
      (size / this.fontMets!.size) * this.fontMets!.distanceRange
  ...
  drawTextBetween(
    ...
    const pos = UIElementRenderer.calculateTextBetweenPosition(
      startXYendXY,
      str,
      this.fontPx,
      scale,
    ...
    const bgColor = UIElementRenderer.getTextBetweenBackgroundColor(
      color,
  ...
  screenFieldOfViewMM(axCorSag = 0, forceSliceMM = false): vec3 {
    ...
    if (this.volumes.length < 1) {
      let mnMM = vec3.fromValues(
        this.extentsMin[0],
        this.extentsMin[1],
        this.extentsMin[2],
      )
      let mxMM = vec3.fromValues(
        this.extentsMax[0],
        this.extentsMax[1],
        this.extentsMax[2],
      )
  ...
  drawSliceOrientationText(
    ...
    if (!isNaN(padLeftTop[0])) {
      ...
      if (padLeftTop[1] > ht) {
        this.drawTextBelow(
          [
            ...
            leftTopWidthHeight[1] + padLeftTop[1] - ht,
          ],
          topText,
        )
        drawBelow = false
      ...
      if (padLeftTop[0] > wid) {
        this.drawTextRight(
          [
            leftTopWidthHeight[0] + padLeftTop[0] - wid,
  ...
  draw2DMain(
    ...
    const flippedY =
      gl.canvas.height - leftTopWidthHeight[1] - leftTopWidthHeight[3]
    ...
    if (this.volumes.length > 0) {
      ...
      shader.use(this.gl)
      gl.uniform1f(
        shader.uniforms.overlayOutlineWidth,
  ...
  drawGraph(): void {
    ...
    const digits = Math.max(
      0,
      -1 * Math.floor(Math.log(spacing) / Math.log(10)),
    )
    ...
    while (lineH <= mx) {
      ...
      this.drawLine(
        [
          ...
          y,
          plotLTWH[0] + plotLTWH[2] + graph.gridLineThickness,
          y,
        ],
        graph.gridLineThickness,
        graph.lineColor,
    ...
    if (
      fntSize > 0 &&
      graph.LTWH[1] + graph.LTWH[3] > plotLTWH[1] + plotLTWH[3] + fntSize * 2.4
    ) {
      this.drawTextBelow(
        [
          plotLTWH[0] + 0.5 * plotLTWH[2],
          plotLTWH[1] + plotLTWH[3] + fntSize * 1.2,
        ],
        "Volume",
        fntScale,
        graph.textColor,
  ...
  drawOrientationCube(
    ...
    mat4.multiply(modelViewProjectionMatrix, projectionMatrix, modelMatrix)
    gl.uniformMatrix4fv(
      this.orientCubeShader!.uniforms.u_matrix,
      false,
      modelViewProjectionMatrix,
    )
  ...
  createOnLocationChange(axCorSag = NaN): void {
    ...
    if (this.volumes.length > 0) {
      ...
      for (let i = 0; i < this.volumes.length; i++) {
        ...
        let flt = this.volumes[i].getValue(
          vox[0],
          vox[1],
          vox[2],
          this.volumes[i].frame4D,
        )
        deci = 3
        if (this.volumes[i].colormapLabel !== null) {
          const v = Math.round(flt)
          if (
            this.volumes[i].hdr.intent_code ===
              NiiIntentCode.NIFTI_INTENT_LABEL &&
            this.volumes[i].hdr.datatypeCode === NiiDataType.DT_RGBA32
          ) {
            const vals = this.volumes[i].getValues(
              vox[0],
              vox[1],
              vox[2],
              this.volumes[i].frame4D,
            ...
            if (vals[2] > 2) {
              ...
              valStr +=
                this.volumes[i].colormapLabel!.labels![vals[0]] + ` (${pct1}%)`
              if (vals[3] > 2) {
                const pct2 = Math.round((100 * vals[3]) / 255)
                valStr +=
                  ` ` +
                  this.volumes[i].colormapLabel!.labels![vals[1]] +
                  ` (${pct2}%)`
              }
            }
          } else if (
  ...
  getLabelAtPoint(screenPoint: [number, number]): NVLabel3D | null {
    ...
    for (const label of this.document.labels) {
      ...
      if (label.anchor & LabelAnchorFlag.MIDDLE) {
        if (
          screenPoint[1] <
          (this.gl.canvas.height - textHeight - verticalMargin) / 2
        ) {
          continue
        }
        ...
        if (
          screenPoint[1] >
          (this.gl.canvas.height + textHeight - verticalMargin / 2) / 2
        ) {
          continue
        }
      }
      ...
      if (label.anchor & LabelAnchorFlag.BOTTOM) {
        if (
          screenPoint[1] <
          this.gl.canvas.height - textHeight - verticalMargin
        ) {
          continue
        }
  ...
  draw3DLabel(
    ...
    if (legendWidth) {
      if (label.style.textAlignment !== LabelTextAlignment.LEFT) {
        const textWidth =
          this.textWidth(label.style.textScale, label.text) * size
  ...
  draw3DLabels(
    ...
    if (panelHeight > this.canvas.height) {
      ...
      this.legendFontScaling =
        Math.max(this.canvas.height - margin, 1) / panelHeight
      // legend too big for screen issue 1279
      log.debug(
        `Legend too large for screen, font reduction factor x${this.legendFontScaling}`,
      )
    ...
    for (const label of labels) {
      ...
      const textHeight =
        this.textHeight(labelSize, label.text) * this.legendFontScaling
  ...
  drawAnchoredLabels(): void {
    ...
    const anchoredLabels = this.document.labels.filter(
      (l) => l.anchor != null && l.anchor !== LabelAnchorPoint.NONE,
    )
    for (const label of anchoredLabels) {
      ...
      this.drawRect(
        [
          left + rectHorizontalOffset,
          top + rectVerticalOffset,
          textWidth + rectWidthDiff,
          textHeight + rectHeightDiff,
        ],
  ...
  draw3D(
    ...
    const glLTWH: [number, number, number, number] = [
      ltwh[0],
      gl.canvas.height - ltwh[3] - ltwh[1],
      ltwh[2],
      ltwh[3],
    ]
  ...
  drawMesh3D(
    ...
    MeshRenderer.drawMesh3D({
      ...
      frac2mm: (pos: number[]) =>
        this.frac2mm(pos as vec3, 0, this.opts.isSliceMM),
      crosshairPos: this.scene.crosshairPos,
    })
  ...
  scaleSlice(
    ...
    const canvasW =
      canvasWH[0] === 0
        ? this.effectiveCanvasWidth() - padPixelsWH[0]
        : canvasWH[0] - padPixelsWH[0]
    const canvasH =
      canvasWH[1] === 0
        ? this.effectiveCanvasHeight() - padPixelsWH[1]
        : canvasWH[1] - padPixelsWH[1]
  ...
  drawThumbnail(): void {
    ...
    const thumb = UIElementRenderer.calculateThumbnailDimensions(
      regionW,
      regionH,
      this.bmpTextureWH,
    )
  ...
  draw3DLine(
    ...
    this.gl.uniform2fv(this.line3DShader.uniforms.canvasWidthHeight, [
      this.gl.canvas.width,
      this.gl.canvas.height,
    ])
    // draw Line
  ...
  drawDottedLine(
    ...
    const segments = UIElementRenderer.calculateDottedLineSegments(
      startXYendXY,
      this.fontPx,
      1.0,
    )
    for (const seg of segments) {
      this.gl.uniform4fv(this.lineShader.uniforms.startXYendXY, [
        seg.startX,
        seg.startY,
        seg.endX,
        seg.endY,
  ...
  drawCrossLinesMM(
    ...
    const { linesH, linesV } = SliceRenderer.getCrossLinesForSliceType(
      axCorSag,
      axiMM,
      corMM,
      sagMM,
  ...
  drawMosaic(mosaicStr: string): void {
    ...
    for (let pass = 0; pass < 2; pass++) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (item.includes("H")) {
          i++
          horizontalOverlap = Math.abs(
            Math.max(0, Math.min(1, parseFloat(items[i]))),
          )
          continue
        }
        if (item.includes("V")) {
  ...
  calculateWidthHeight(
    ...
    return SliceRenderer.calculateSliceDimensions(
      sliceType as SLICE_TYPE,
      volScale,
      containerWidth,
      containerHeight,
    )
  ...
  drawSceneCore(): string | void {
    ...
    const shouldUpdateClipDrag = ClipPlaneManager.shouldUpdateClipPlaneDrag({
      ...
      dragStartTileIndex: this.inRenderTile(
        this.uiData.dragStart[0],
        this.uiData.dragStart[1],
      ),
    })
      ...
      if (this.customLayout && this.customLayout.length > 0) {
        ...
        for (const view of this.customLayout) {
          ...
          if (leftTopWidthHeight[1] + leftTopWidthHeight[3] > vpY + vpH) {
            log.warn(
              "adjusting slice height because it would have been clipped",
            )
        ...
        const pad =
          parseFloat(`${this.opts.multiplanarPadPixels}`) * this.uiData.dpr
        ...
        function padPixelsWH(cols: number, rows: number): [number, number] {
          return [
            (cols - 1) * pad + cols * innerPad,
            (rows - 1) * pad + rows * innerPad,
          ]
        }
        ...
        let canvasWH: [number, number] = [
          this.effectiveCanvasWidth(),
          this.effectiveCanvasHeight(),
        ]

        if (
          this.opts.heroImageFraction > 0 &&
          this.opts.heroImageFraction < 1
        ) {
          isShowRender = false
          isHeroImage = true
          if (
            canvasWH[0] > canvasWH[1] &&
            this.opts.multiplanarLayout !== MULTIPLANAR_TYPE.ROW
          ) {
          ...
          canvasWH = [
            canvasWH[0] - heroImageWH[0],
            canvasWH[1] - heroImageWH[1],
          ]
        }
        ...
        if (isHeroImage) {
          ...
          if (
            ...
            this.draw2D(
              [regionX, regionY, heroW, heroH],
              this.opts.heroSliceType,
              NaN,
              [Infinity, Infinity],
            )
          } else {
        ...
        if (isDrawColumn) {
          this.draw2D(
            [regionX + ltwh[0], regionY + ltwh[1], sX, sY],
            SLICE_TYPE.AXIAL,
            NaN,
            [actualX, actualY],
          )
          this.draw2D(
            [regionX + ltwh[0], regionY + ltwh[1] + sY + pad, sX, sZ],
            SLICE_TYPE.CORONAL,
            NaN,
            [actualX, actualZ],
          )
          ...
          this.draw2D(
            [regionX + ltwh[0] + sX + pad, regionY + ltwh[1], sX, sZ],
            SLICE_TYPE.CORONAL,
            NaN,
            [actualX, actualZ],
          )
          this.draw2D(
            [regionX + ltwh[0] + sX + sX + pad * 2, regionY + ltwh[1], sY, sZ],
            SLICE_TYPE.SAGITTAL,
            NaN,
            [actualY, actualZ],
          )
          ...
          this.draw2D(
            [regionX + ltwh[0], regionY + ltwh[1] + sZ + pad, sX, sY],
            SLICE_TYPE.AXIAL,
            NaN,
            [actualX, actualY],
          )
          this.draw2D(
            [regionX + ltwh[0], regionY + ltwh[1], sX, sZ],
            SLICE_TYPE.CORONAL,
            NaN,
            [actualX, actualZ],
          )
          this.draw2D(
            [regionX + ltwh[0] + sX + pad, regionY + ltwh[1], sY, sZ],
            SLICE_TYPE.SAGITTAL,
            NaN,
            [actualY, actualZ],
          )
          if (isDraw3D) {
            this.draw3D([
              regionX + ltwh[0] + sX + pad,
              regionY + ltwh[1] + sZ + pad,
              sY,
              sY,
            ])
          }
    ...
    if (this.uiData.isDragging) {
      ...
      if (
        this.inRenderTile(this.uiData.dragStart[0], this.uiData.dragStart[1]) >=
        0
      ) {
        return
      }
      ...
      const currentDragMode = this.getCurrentDragMode()
      if (
        currentDragMode === DRAG_MODE.contrast ||
        currentDragMode === DRAG_MODE.roiSelection
      ) {
    ...
    for (const measurement of this.document.completedMeasurements) {
      if (
        this.shouldDrawOnCurrentSlice(
          measurement.sliceIndex,
          measurement.sliceType,
          measurement.slicePosition,
        )
      ) {
        ...
        const startCanvasResult = this.frac2canvasPosWithTile(
          startFrac,
          measurement.sliceType,
        )
        const endCanvasResult = this.frac2canvasPosWithTile(
          endFrac,
          measurement.sliceType,
        )
        ...
        if (
          startCanvasResult &&
          endCanvasResult &&
          startCanvasResult.tileIndex === endCanvasResult.tileIndex
        ) {
          this.drawMeasurementTool([
    ...
    for (let i = 0; i < this.document.completedAngles.length; i++) {
      ...
      const shouldDraw = this.shouldDrawOnCurrentSlice(
        angle.sliceIndex,
        angle.sliceType,
        angle.slicePosition,
      )
      ...
      if (shouldDraw) {
        ...
        const firstLineStartCanvasResult = this.frac2canvasPosWithTile(
          firstLineStartFrac,
          angle.sliceType,
        )
        const firstLineEndCanvasResult = this.frac2canvasPosWithTile(
          firstLineEndFrac,
          angle.sliceType,
        )
        const secondLineStartCanvasResult = this.frac2canvasPosWithTile(
          secondLineStartFrac,
          angle.sliceType,
        )
        const secondLineEndCanvasResult = this.frac2canvasPosWithTile(
          secondLineEndFrac,
          angle.sliceType,
        )
        ...
        if (
          ...
          firstLineStartCanvasResult.tileIndex ===
            firstLineEndCanvasResult.tileIndex &&
          firstLineStartCanvasResult.tileIndex ===
            secondLineStartCanvasResult.tileIndex &&
          firstLineStartCanvasResult.tileIndex ===
  ...
  get gl(): WebGL2RenderingContext {
    if (!this._gl) {
      throw new Error(
        "unable to get WebGL context. Maybe the browser doesn't support WebGL2.",
      )
    }
    return this._gl
  }
